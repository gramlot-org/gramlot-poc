...
### `genropy/gnrpy/gnr/web/widgets/dojox.py::DojoxWidgets::fileInputBlind:L195`
...
- `genropy/gnrpy/gnr/web/widgets/dojox.py::DojoxWidgets::fileInputBlind:L195`: File input that shows only a button (no visible text field), with background upload. Args: url: URL for background file upload. blurDelay: Time in ms before upload starts after losing focus. duration: Animation duration in ms. uploadMessage: Text shown during upload progress. label: Text label on the browse button. cancelText: Text label on the cancel button. name: Name attribute for the file input field.
