...
### `genropy/gnrpy/gnr/web/widgets/dijit.py::DijitWidgets::numberTextBox:L51`
...
| `constraints` | dict \| None | optional | `None` | Legacy Source binding syntax may be accepted; exact consumer must be followed. | Dict with min, max, places, pattern constraints. |
...
- `genropy/gnrpy/gnr/web/widgets/dijit.py::DijitWidgets::numberTextBox:L51`: A validating, serializable, range-bound text box for numbers. Args: value: Current numeric value. constraints: Dict with min, max, places, pattern constraints. required: If True, the field must be filled in. invalidMessage: Message displayed when the value is invalid. keypad: If True, an icon in the field opens a numeric keypad popup. keypad_calculator: If True, that popup is a calculator; implies keypad. keypad_size: 'large' for a coarser touch target on the popup keys.
...
- Current Gramlot name-level evidence: `js/dom/src/components/builtin-components.json`, `src/gramlot/grammar/inputs.py:L45`.
